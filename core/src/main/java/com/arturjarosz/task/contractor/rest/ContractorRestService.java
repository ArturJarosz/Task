...
public class ContractorRestService {
    ...
    public ResponseEntity<ContractorDto> createContractor(@RequestBody ContractorDto contractorDto) {
        ContractorDto createdContractor = this.contractorApplicationService.createContractor(contractorDto);
        HttpHeaders headers = new HttpHeadersBuilder().withLocation("/contractors/{contractorId}", createdContractor.getId()).build();
