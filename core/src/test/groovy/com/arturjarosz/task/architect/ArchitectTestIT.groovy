...
class ArchitectTestIT extends BaseTestIT {
    ...
    def "Calling /get with existing Architect id should return ArchitectDto with proper data"() {
        ...
        when:
            def response = this.mockMvc.perform(
                    MockMvcRequestBuilders
                            .get(URI.create(HOST + ":" + port + ARCHITECTS_URI + "/" + createdArchitectDto.getId()))
                            .header("Content-Type", "application/json")
            ).andReturn().response;
        then:
            response.status == HttpStatus.OK.value();
        and:
            ...
            architect.getFirstName() == FIRST_NAME;
            architect.getLastName() == LAST_NAME;
    }
    ...
    def "Updating existing Architect with proper data should update Architect and return updated ArchitectDto"() {
        ...
        when:
            def updatedArchitectString = this.mockMvc.perform(
                    MockMvcRequestBuilders
                            .put(URI.create(HOST + ":" + port + ARCHITECTS_URI + "/" + createdArchitectDto.getId()))
                            .header("Content-Type", "application/json")
        ...
        and:
            ...
            architect.getFirstName() == NEW_FIRST_NAME;
            architect.getLastName() == NEW_LAST_NAME;
    }
    ...
    private ArchitectDto prepareArchitectDto(String firstName, String lastName) {
        ...
        architectDto.setFirstName(firstName);
        architectDto.setLastName(lastName);
        return architectDto;
    }
}
